...
- **Story 1.1: Backend Foundation (Node.js & Express/NestJS)**
  - **Description:** Set up a Next.js REST API with TypeScript. This includes configuring the project structure, environment variables (`.env`), and a middleware pipeline for logging (Winston/Morgan) and error handling.
...
- **Story 1.5: Health Endpoints**  
  - **`GET /api/ping`**: Responds with a status of 200 to caller to indicate that the service is up.
...
- **Story 2.1: User Registration**
  ...
  - **Description:** As a new user, I want to register for an account using my email and password, so that I can securely access the system. The password is stored as a one-way hash in the database.
...
- **Story 2.3: JWT-Based Authentication**

  - **Description:** Implement secure login/logout using JSON Web Tokens (JWT). Include password hashing (bcrypt) and refresh token logic in the NextJS backend.

- **Story 2.4: Password Reset Functionality**
...
- **Story 2.5: Permission Middleware**
...
- **Story 2.7: Role-Based Route Protection**
...
- **Story 2.8: User Profile**

  - **Description:** As a user, I want to get my profile 

  - **`GET /api/v1/users/profile`**: Allows users to get their personal details and security settings.
  - **`PUT /api/v1/users/profile`**: Allows users to update their personal details and security settings.
